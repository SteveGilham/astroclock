...
let localsolarnoon prev longitude =
  ...
  let UT_Sun_in_south = UT_Sun_in_south' + (if UT_Sun_in_south' < 0.0 then 1.0 else 0.0)
  printfn "%A" UT_Sun_in_south
  ...
  let ticks = new System.TimeSpan(int64(UT_Sun_in_south * (float System.TimeSpan.TicksPerDay)))
  printfn "%A" ticks
