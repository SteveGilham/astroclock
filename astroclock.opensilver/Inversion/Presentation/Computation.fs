...
module Computation =
  ...
  type Display = {Latitude:float<deg>;
                  Longitude:float<deg>;
                  CentreX:float;
                  CentreY:float;
                  Radius:float}
  ...
  let localsolarnoon prev longitude =
    ...
    let utSuninsouth' = ( (ra*180.0<deg>/Math.PI) - (getGMST0 quant) - longitude ) / 360.0<deg>
    let utSuninsouth = utSuninsouth' + (if utSuninsouth' < 0.0 then 1.0 else 0.0)
    printfn "%A" utSuninsouth
    let utcnow = System.DateTimeOffset.UtcNow
    let utczero = System.DateTimeOffset(prev.Year, prev.Month, prev.Day, 0,0,0, (TimeSpan(int64 0)) )
    let ticks = new System.TimeSpan(int64(utSuninsouth * (float System.TimeSpan.TicksPerDay)))
    printfn "%A" ticks
  ...
  let geoEr planet date display =
    ...
    let alttopoc = alt - (asin dimensionless) * (cos alt)
